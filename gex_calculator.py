...
def calculate_gex_for_expiration(session, crumb, ticker, spot, exp_ts, risk_free=0.04):
    ...
    for c in calls:
        ...
        # Improvement #1: apply DTE weight
        call_gex = oi * gamma * 100 * spot * weight

        # Improvement #5: charm for 0DTE (annualized → per-day unwind pressure)
...
def calculate_full_gex(ticker, num_expirations=6, session_crumb=None):
    ...
    total_gex_by_strike = {}
    zero_dte_gex_by_strike = {}  # track 0DTE separately for charm reporting
    ...
    if not nearby:
        nearby = total_gex_by_strike  # fallback if nothing in range
    ...
    # Improvement #4: cumulative GEX flip via zero-crossing of cumulative sum
    ...
    # Improvement #5: total charm exposure (0DTE unwind pressure)
    total_charm_gex = sum(
